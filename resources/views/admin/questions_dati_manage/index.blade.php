...
<div class="row black">
    ...
    <div class="x_panel">
        ...
        @include('common._alert')
            <span class="section">
            <a href="{{ url('admin/questionDatiManage/create')}}" ><button type="button" class="btn btn-success">新增</button></a>
...
function initTable() {
    var table = $('#table').DataTable( { //初始化表格
        ...
        // ajax请求
        "ajax": {
            'url' : 'questionDatiManage/table', //请求地址
            //传递额外参数 (条件搜索)
            // 'data' : function (d) {
            //     d.test = 1;
            // }
        ...
        "aoColumns": [{ "mData": null,
              ...
              "sWidth" : "10%",
                // 返回自定义内容
                "render": function(data, type, full) {
                    return "<a type='button' class='btn btn-warning btn-sm' href='/admin/questionDatiManage/" + full.id + "/edit'>修改</a>&nbsp;<a type='button' class='delete btn btn-danger btn-sm' href='javascript:void(0);' >删除</a>";
                }
            },
        ],
    ...
    $('#delete').click( function () {
       ...
       if(ids.length<1){
           ...
           layer.msg('确定删除这些项目？', {
            ...
            ,yes: function(index){
              layer.close(index);
              var url = '/admin/questionDatiManage/destroy_many';
